...
export default {
  ...
  created() {
    ...
    this.initCoupopnForm()
  },
  methods: {
    initCoupopnForm() {
      couponApi.getCouponInfo(this.couponId).then((res) => {
        this.couponForm = res.data
        this.coupopnFormBak = JSON.parse(JSON.stringify(res.data))
        this.categoryRelationList = this.couponForm.productCategoryRelationList
        this.productRelationList = this.couponForm.productRelationList
        this.generateTable(this.couponForm)
      })
    },
    generateTable(obj) {
      if (obj.useType === 1) {
        let list = []
        for (const iterator of obj.productCategoryRelationList) {
          list.push({
            id: iterator.productCategoryId,
            name:
              iterator.parentCategoryName + '/' + iterator.productCategoryName,
          })
        }
    ...
    submitForm() {
      this.$refs.ruleForm.validate((valid) => {
        ...
        if (
          this.couponForm.useType === 1 &&
          this.categoryRelationList.length === 0
        ) {
          return this.$message.warning('请选择指定分类')
        }
        if (
          this.couponForm.useType === 2 &&
          this.productRelationList.length === 0
        ) {
        ...
        couponApi.updateCoupon(this.couponForm).then((res) => {
    ...
    resetForm() {
      // 这里的重置表单回到最开始的数据
      this.couponForm = JSON.parse(JSON.stringify(this.coupopnFormBak))
      this.categoryRelationList = this.couponForm.productCategoryRelationList
      this.productRelationList = this.couponForm.productRelationList
      this.generateTable(this.couponForm)
    },
